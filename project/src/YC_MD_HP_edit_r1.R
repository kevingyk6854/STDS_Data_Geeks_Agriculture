...
yield_by_region_1 <- yield_by_region_1[-222,]
yield_by_region_1 <- yield_by_region_1[-180,]
# Change Charactor Data to Factors
yield_by_region_1=yield_by_region_1 %>% mutate_if(is.character, as.factor)
glimpse(yield_by_region_1)
...
# Predict Test Set
test_feat <- select(test, -yield)
test$predict=predict(model1,test_feat)


# Predict Train Set
train_feat <- select(train, -yield)
train$predict=predict(model1,train_feat)
