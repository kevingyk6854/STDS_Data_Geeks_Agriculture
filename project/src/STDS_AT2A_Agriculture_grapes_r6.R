...
variety_irrigation <- read_csv(here::here("project/src/data/variety_irrigation", "variety_irrigation.csv"))
...
register_google(key = "AIzaSyAcfmC-fAerEw3Gn4Pg23KAX7BOWEjKWlY")
geocodes <- geocode(variety_irrigation$gi_region)
variety_irrigation_points <- SpatialPointsDataFrame(coords = geocode_check, data = variety_irrigation, proj4string = sp::CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
